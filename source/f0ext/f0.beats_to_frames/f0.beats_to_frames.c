...
int C74_EXPORT main(void) {
	t_class *c= class_new("f0.beats_to_frames", (method)f0ext_new, (method)NULL, (long)sizeof(t_f0ext), 0L, A_GIMME, 0);
	...
	f0ext_class= c;
	post("f0.beats_to_frames v2.01-64/32bit; distributed under GNU GPL license");
    return 0;
}
void *f0ext_new(t_symbol *s, long argc, t_atom *argv) {
...
void theFunction(t_f0ext *x) {
	if(x->valMiddle==0.0) {
		...
		x->valOut= (60.0/x->valMiddle)*x->valLeft*x->valRight;
	}
	f0ext_bang(x);
}
